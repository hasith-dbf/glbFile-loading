...
export class Viewer {
  ...
  brushSelectedObject(selectedMeshObject, x, y, z) {
    ...
    let result = {};
    const angleToRotateZ = (3 * Math.PI) / 4;
    const angleToRotateX = Math.PI / 3 + Math.PI / 2;
    selectedMeshObject.position.x -= x;
    selectedMeshObject.position.y += y;
    selectedMeshObject.position.z += z;
    if (selectedMeshObject) {
      selectedMeshObject.rotateX(angleToRotateX);
      selectedMeshObject.rotateZ(angleToRotateZ);
      ...
      result = csgEvaluator.evaluate(brushes[1], brushes[0], SUBTRACTION);
      return result;
    }
  }
  setContents(objects, clips) {
    ...
    objects.map((object, index) => {
      ...
      const angleToRotateZ = (3 * Math.PI) / 4;
      const angleToRotateX = Math.PI / 3 + Math.PI / 2;
      object.rotateX(angleToRotateX);
      object.rotateZ(angleToRotateZ);
      // switch (index) {
      //     case "1": {
      //         object.position.x += object.position.x - center.x;
      //         object.position.y += object.position.y - center.y;
      ...
      const selectedMeshObject = meshObjects[24].children[0].clone();
      ...
      const res5 = this.brushSelectedObject(
        selectedMeshObject3.children[1].clone(),
        23,
        11,
        8
      );
      const res6 = this.brushSelectedObject(
        selectedMeshObject4.children[1].clone(),
        9.5,
        11,
        -20
      );
